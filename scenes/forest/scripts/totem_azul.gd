...
func _ready():
	...
	"2" : [""],
	"3": ["Ativação(){"] ,
	"4" : ["\tif(TotemVerde.ativo && TotemVermelho.ativo && TotemAmarelo.ativo)"],
	"5" : ["\t\tativado = true;"],
	"6" : ["\t}else{"],
	"7" : ["\t\tTotemAmarelo.ativo = false;"],
...
func animation_totem()->void:
	if ativado:
		...
		tween.tween_property(self,"modulate:b",1,0.2)
		ativado = true
		codigo = {"boolean ativado": ativado}
		forest_scene.finish_puzzle()
	else:
		var tween:Tween = create_tween()
		tween.tween_property(self,"modulate:b",0.58,0.2)
		ativado = false
		codigo = {"boolean ativado": ativado}
