...
func starting_cutscene():
	...
	#tween.tween_callback(player.set_animation.bind("idle", "left"))
...
func _on_textbox_text_finish():
	#var tween = create_tween()
	#tween.tween_callback(kath.set_sprite.bind("idle_left"))
	#tween.tween_property(kath, "position", Vector2(6400,7417), 1.6).set_trans(Tween.TRANS_LINEAR)
	#tween.tween_callback(kath.set_sprite.bind("idle_up"))
	#tween.tween_property(kath, "position", Vector2(6400,4000), 1.6).set_trans(Tween.TRANS_LINEAR)
	#tween.tween_callback(player.set_in_scene.bind(false))
	#tween.tween_callback(kath.set_position.bind(Vector2(4500, -5000)))
